import MovieDetailSkeleton from '@/components/skeleton/MovieDetailSkeleto'
    ...
    <>{loading? <MovieDetailSkeleton />:
      <div className='relative w-full pt-30'>
        ...
        <div className='absolute -z-20 top-0 right-0 left-0 h-[100vh] overflow-hidden'>
          <img className='object-cover object-right' src={ORG_IMG_URL + film?.horizontalPostURL}></img>
        </div>
        ...
        <div className='pt-5 md:pt-10 px-5 md:px-20 lg:px-40 w-full'>
              ...
              <div className='px-10 md:px-40 grid grid-cols-2 lg:grid-cols-4 gap-5'>
                {recommand?.map((film) => {
                  return (
                      <div onClick={() => {navigate(`/movie/${film.filmid}`); scrollTo(0,0) }} className='group/one px-2 py-2 flex flex-col items-center cursor-pointer transition-all duration-500' key={film.filmid}>
                          <img className='max-h-[250px] max-w-[160px] object-cover object-center group-hover/one:scale-104 transition-all duration-500 shadow-[0_0_50px_black]' src={ORG_IMG_URL + film.verticalPostURL}></img>

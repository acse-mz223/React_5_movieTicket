...
function SeatSelection() {
  ...
  async function paymentProcess() {
    ...
    try{
      const session = await axios.post("/api/frontend/createCheckoutSession", {
        seat: selectedseat,
        price: price
      })

      setClientSecret(session.data.content)
    }catch(error){
  ...
  async function checkout() {
    try{
      ...
      if (!user.isSignedIn) {
        ...
        paymentProcess() 
        console.log("check out session created successfully")
      }
    }catch(error){
    ...
    <div className='w-full pt-30'>
      ...
      <div className='relative px-5 md:px-20 lg:px-40 py-4 lg:py-10 flex flex-col md:flex-row md:justify-between gap-5 before:absolute before:content-[""] before:top-0 before:left-0 before:right-0 before:bottom-0 before:bg-gradient-to-b before:from-black before:via-white/8 before:to-black before:-z-10'>
        ...
        <div className='flex flex-row items-center gap-5'>
          ...
          <Button disabled={seatNumber === 1 || clientSecret } onClick={() => {if(selectedseat.length === seatNumber) {setselectedSeat((pre) => [...pre.slice(1)])};setSeatNumber((pre)=> pre-1);}} className='rounded-full size-10 cursor-pointer'><MinusIcon className='size-7'></MinusIcon></Button>
          <div className='text-9xl'>{seatNumber}</div>
           <Button disabled={seatNumber === 10 || clientSecret} onClick={() => {setSeatNumber((pre)=> pre+1)}} className='rounded-full size-10 cursor-pointer'><PlusIcon className='size-7'></PlusIcon></Button>
        </div>
      </div>
      {/* book board */}
      <div className='md:h-[600px] py-10 px-5 md:px-20 lg:px-40 flex md:flex-row flex-col gap-10 lg:gap-20'>
        {/* seat map */}
        ...
        <div className='w-full'>
          ...
          >
            <PaymentForm showtimeInfo={showtimeInfo} selectedseat={selectedseat} price={price}/>
          </Elements>
        </div>
        }
      </div>
